...
(function () {
  ...
  function render() {
    ...
    bubble.innerHTML = `
      ...
      <form class="ask"><input id="faris-q" type="text" maxlength="300" placeholder="${t("askFaris")}" autocomplete="off"><button class="link" type="submit">${t("send")}</button></form>`;
    ...
    bubble.querySelector("form").onsubmit = async (e) => {
      ...
      say(t("farisThinking"));
      try {
        const r = await fetch("/api/faris/ask", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ question: q }) }).then((r) => r.json());
  ...
  window.Faris = { mount, say, draw, hide: () => { if (root) root.hidden = true; }, show: () => { if (root) root.hidden = false; }, isOpen: () => state.open, rerender: render };
})();
