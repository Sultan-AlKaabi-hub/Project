...
(function () {
  ...
  function render() {
    ...
    bubble.querySelector(".ask").onsubmit = async (e) => {
      ...
      try {
        const context=window.App?.getContext?.()||{};
        let r;
        if(window.LearningAI && context.hasAI!==false){
          let streamed='';
          r=await window.LearningAI.request(q,context,chatLang,{signal:AbortSignal.any([pending.signal,AbortSignal.timeout(75000)]),onRoute:label=>{if(version===requestVersion)bubble.querySelector('.msg').textContent=label+'…';},onDelta:text=>{streamed+=text;if(version===requestVersion)bubble.querySelector('.msg').textContent=streamed;}});
        }else{
