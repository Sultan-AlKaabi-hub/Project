...
export async function respond(db,user,input,{signal,article=null,onText=()=>{},onStatus=()=>{}}={}){
 ...
 // Recent tab conversation is untrusted dialogue, not authority, and is not persisted unless history is enabled.
 if(Array.isArray(input.conversation)&&!privateResult){context.history=input.conversation.slice(-3).filter(t=>typeof t?.question==='string'&&typeof t?.answer==='string').map(t=>({question:clean(t.question).slice(0,1000),answer:clean(t.answer).slice(0,2000)}));}
 ...
 if(provider.enabled){try{const generated=await provider.stream(SYSTEM+rolePrompt(routing.agent),context,{signal,onText,task:routing.agent==='review'?'review':'tutor'});if(generated?.text){text=generated.text;model=generated.model;usage=generated.usage;cards=cards.filter(c=>c.type!=='explanation'&&c.type!=='hint');}}catch(e){if(signal?.aborted)throw e;error='provider_unavailable';}}
