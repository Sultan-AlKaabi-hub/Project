...
export function createSupabaseStore(env=process.env,request=fetch){
 ...
 let revision=0;
 async function call(path,body){const r=await request(origin.origin+'/rest/v1/'+path,{method:body?'POST':'GET',headers,body:body?JSON.stringify(body):undefined,signal:AbortSignal.timeout(20000)});if(!r.ok)throw new Error('Durable storage unavailable ('+r.status+')');return r.json();}
